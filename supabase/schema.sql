...
create unique index if not exists characters_name_normalized_key on public.characters(name_normalized);
...
declare c public.characters; t uuid;
begin
 select * into c from public.characters where name_normalized=lower(btrim(p_name));
...
begin
 if not exists(select 1 from public.sessions where token=p_token and character_id=p_character_id and not is_host) then return jsonb_build_object('ok',false); end if;
 update public.characters set sheet=p_sheet,updated_at=now() where id=p_character_id returning updated_at into u;
...
begin
 ...
 return jsonb_build_object('ok',true,'characters',coalesce((select jsonb_agg(jsonb_build_object('id',id,'name',name,'updated_at',updated_at) order by updated_at desc) from public.characters),'[]'::jsonb));
end $$;
